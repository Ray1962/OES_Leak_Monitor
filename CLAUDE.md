...
- `RatioReviewViewModel` + `RatioReviewPanel` + `RatioCsvReader` — Ratio Review tab: scans the data folder for the `{prefix}_Ratio_*.csv` files `RatioCsvLogger` writes, and plots the selected file's per-ratio trend either as % of baseline (with the 100/120/150 % threshold lines) or as the raw ratio, with the composite leak state drawn as translucent Warning/Alarm bands behind the series. Reuses `Recording.TryParse` for the shared filename scheme, filtering on the `"Ratio"` tag. PNG export, clipboard copy, search.
