...
- 可以在 **Replay** 分頁選一個已錄製的全光譜 CSV 來取代合成波形，用真實電漿資料驗證偵測演算法（見 §6.7 與 §8.5）。
...
已經搬到專屬的 **Replay** 分頁（見 §6.7）。驗證演算法需要的是播放控制、倍速與進度，不是一個選檔按鈕。底部動作列這裡只留一行提示。
...
把一份**已錄製的全光譜 CSV** 當成即時光譜餵進整條流程 —— Leak Monitor、兩張趨勢圖、以及兩個記錄器都會收到 —— 用來在**沒有硬體**的情況下驗證偵測演算法。與 §6.8 Recordings 的差別：Recordings 是把資料畫出來給人看，Replay 是把資料**重新跑一遍給程式判斷**。
