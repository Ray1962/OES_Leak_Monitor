...
7. **若跳出視窗，先看完再繼續**：警告視窗代表有比值沒拿到基準線（那幾條不會參與警報）；錯誤視窗代表一條都沒拿到，這次擷取已被丟棄、舊基準線原封不動 —— 依視窗上的原因排除後重錄（見 §6.3.5 與 §9.5）。
...
6. 只要**至少有一條比值**取得基準線，這次擷取就會**立即**寫入 `settings.json` 並自動設為使用中的基準線。
