...
- 同時會記錄 **PlasmaPresentFloor**（＝參考譜線基準均值的 **20%**），之後用來判斷「電漿有沒有開」（僅 `Ratio` 模式使用）。
...
> 第二欄的名稱會說明它裝的是什麼：一般是 `R_x_pctBaseline`（% 基準）；若該比值採 `AbsoluteIntensity` + `RawMean`，就會是 **`R_x_sigmaScore`**（σ 分數）。兩者都畫在同一組 100/120/150 標尺上，但**不是同一個量**，所以 Ratio Review 會在圖例標上 `(σ-score)`。
...
要保存一台機台的完整組態，備份 **`settings.json`** 一個檔就夠了 —— 裡面包含所有裝置參數、記錄器設定、比值定義、Golden Run 基準線、洩漏率校正、波長修正與資料保存設定。
...
實測（本機、`AnyMonitoredWavelength`，監控 436.8 / 487.4 / 546.6 nm）：
...
門檻 **10000** 約為電漿開的 1/3、電漿關的 10 倍，兩邊都有很大餘裕。**這是你機台的數字，不是通則** —— 換配方、換積分時間、清過觀察窗之後都要重新量。量法：把門檻欄位下方的綠色小字（`now …·threshold …`）分別在電漿開與關的時候各看一次，取中間值。
