...
動作列上方還有一排 **Data folder** 設定（舊資料自動壓縮與磁碟用量），見 §7.4。
...
> **絕對強度模式的「電漿存在」判定來自記錄器的儲存觸發**（Configuration 分頁的 trigger 波長與 save-start 門檻），不再看參考譜線 —— 選了這個模式，右側就不會出現 Reference line 欄位。
>
