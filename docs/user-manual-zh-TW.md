...
>
> 教科書上的 actinometry 多半拿 **O 777.2 / Ar** 當空氣洩漏指標，本軟體的出廠預設比值（§10.5）也是照這個假設配的。但在一台 **Ar 80 sccm、0.95 torr、RF 50 W** 的實機上實測：**O 777.2 對 100 單位的空氣洩漏只有 0.7σ 的反應（等於沒有），而 N₂ 337.1 有 33.8σ** —— 分子與分母的角色完全反了過來。
>
...
> ⚠️ **這些是未經你的機台驗證的起點，不是建議值。** 它們依據的是「空氣洩漏會抬高 O 的發光」這個教科書假設 —— 而在實測過的那台 Ar 電漿上，這個假設不成立（O 777.2 對 100 單位洩漏只有 0.7σ，OH 309 是 0.1σ，連 N₂ 337.1 都被放在分母）。
>
> **上線前請依 §1.1 用一次已知洩漏挑出你機台真正會動的譜線**，再據此重設比值。那台機器最後採用的是：

| 角色 | 比值 | 訊號線 | 擷取方式 | 參考線 |
|---|---|---|---|---|
| 主指標 | N₂ 337.1 / Ar 750.4 | N₂ 337.1 nm | RawMean（半寬 1.0） | Ar 750.4 nm（PeakHeight） |
| 次指標 | NO 237 / Ar 750.4 | NO 237.0 nm | RawMean | Ar 750.4 nm（PeakHeight） |

> 門檻倍數也一併調整為 Warn 1.05 / Alarm 1.12（σ 3 / 6 不變）—— 出廠的 1.2 / 1.5 配上 2% 的基準散度，會讓乘法那一支永遠勝出而把 σ 門檻整個蓋掉，詳見 §6.4.2。
