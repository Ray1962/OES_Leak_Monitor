...
| 可偵測(保守,E2 的 k) | **6.09×10⁻⁴ mbar·L/s** → **餘裕 14.6 倍** |
| 可偵測(E3 的 k) | 2.95×10⁻⁴ mbar·L/s → 餘裕 30.1 倍 |

規格大小的漏氣讓 `R_N2CO_C` 上升 **+136 %～+280 %**(讀值變兩到四倍),
而不是判準要求的 9.3 %。

**餘裕的意義不在於好看,而在於它吸收得掉 §15.4 那條沒有量化的風險**:
就算 TEOS 把 k 砍掉一個數量級,規格大小的漏氣仍然偵測得到。這是這次判定唯一真正的軟肋,
而 14.6 倍是它的預算。

### 16.2 洩漏率靈敏度(P5 用)

    x = s × Q,   s = 153（E2 的 k）～ 315（E3 的 k）  per (mbar·L/s)

x 是 `R_N2CO_C` 相對無漏基準的**分數上升量**。這個數字可以拿來**驗算**日後真正做出來的
`LeakCalibration`:實測 s 若落在 150～320 之外,不是校正做錯,就是這裡的外推假設錯了。
...
| σ 的種類 | 值 | 誰在用 |
|---|---|---|
| **逐幀**(窗口內每幀的散度) | **5.2 %**(四分位 4.5 / 6.5 %) | Golden Run 的 σ、即時 EWMA σ → 逐幀門檻 |
| **跨批次**(取樣點的中位數之間) | **3.1 %**(§3) | Batch Trend 的穩健帶 |

於是兩條路徑的門檻可以翻譯成漏率(用保守的 s = 153):

| 路徑 | 規則 | 讀值上升 | 對應 Q | 相對規格 |
|---|---|---|---|---|
| **跨批次(主力)** | 連續三批 > 3 σ_batch | 9.3 % | 6.1×10⁻⁴ | **15 倍以下** |
| 逐幀 Warning | `sigmaWarn 3` × 5.2 % | 15.7 % | 1.0×10⁻³ | 8.7 倍以下 |
| 逐幀 Alarm | `sigmaAlarm 6` × 5.2 % | 31.4 % | 2.1×10⁻³ | 4.3 倍以下 |
...
2. **`warnFactor 1.05` / `alarmFactor 1.12` 是失效的。** 引擎取
   `max(factor × mean, mean + n·σ)`,而 σ = 5.2 % 讓 σ 項在兩級都勝出
   (要 σ 掉到 1.67 % 以下 factor 才會綁住)。**留著它們當下限沒有壞處,但調它們不會有任何效果** ——
   下一個想「把警報調鬆一點」的人會先動這兩個數字,然後發現沒反應。要動的是 `sigmaWarn`。
3. **跨批次路徑比逐幀路徑靈敏 1.7 倍**,而且不受批次內結垢影響。
   主力仍然是 Batch Trend;逐幀路徑是粗篩,不是主判。
