...
>
> ### ⚠️ 2026-09-04 更新:這一整段的算術已經被 §15 取代
>
> 腔體實際是 **400 L**,不是 50 L,所以 `Q_min = 1×10⁻⁴` 這個數字是錯的 ——
> 同樣的 rate-of-rise 規格套在大八倍的腔體上,允許的漏率就大八倍。
> 而且**受控漏氣測試已經在實驗機台上做完了**(§15),靈敏度不必再上生產機台量。
> 現在缺的只剩兩個數字:**生產機台製程 C 的總氣體流量**,與**該機台實際的漏率驗收規格**。
...
## 10. P2 — 批次層(2026-09-03)

`BatchTracker`(純計算)+ `BatchCsvLogger`(寫檔),兩者都不碰 `RatioMonitor`。

**為什麼是獨立的一層。** 每幀的監測器回答「現在有沒有在漏」,對照的是一條 baseline。這台機真正要問的是
「這一批跟前面四十批比起來怎麼樣」,而那不能逐幀問:視窗在每個批次內都會結垢、清洗後才復原,所以
同一批次內不同時間點的兩幀不可比,而**不同批次的同一個時間點可比**。批次層就是把每個批次縮成
撐得住這個比較的那幾個數字。

它**只用中位數,不用 EMA、不用 liveσ** —— 這正是它對 §9.3 那個效應免疫的原因(階躍會抬高
liveσ、把自己的警報門檻一起帶上去)。這個切分方式與 `RatioCsvLogger` 當初從 intensity logger
獨立出來,理由完全相同。

用中位數不用平均:光譜儀會回傳零星空白幀,gate 會擋掉,但十個點的平均仍會被一個貼著地板的殘存值拉走,
中位數不會。

### 10.1 取樣點

**每批次、每個 class 的第一支 complete step,gate 開啟後 10–30 s 的中位數。**

「第一支」不是「最好的一支」也不是「平均」:同一批次裡同一段製程的兩支 step 處在視窗結垢的不同階段,
把它們平均掉等於在量鍍膜。10–30 s 的理由見 §2.1。窗口內幀數不足 `MinWindowFrames`(5)的 step
會被記錄但標成 incomplete —— 缺一個數字是可讀的,一個站不住的數字不是。

### 10.2 批次邊界

兩條規則,或的關係:

1. **間隔 > `BatchGapSeconds`(240 s)。** 實測批次內的間隔是 14–178 s,批次間是 332 s 以上。
2. **`BatchStartClass` 的 step 且時長 ≥ `BatchStartMinDurationSeconds`(120 s)。**
   間隔規則是一個門檻,這一條是一個事實。設成 `B` / 120 s 就對應「每批以 156 s 的清洗開場」,
   而同一個 class 也會在批次中間跑 36 s —— 所以光看 class 不夠,**時長才是分界**。

判定在 step **結束**時做,因為第 2 條需要時長。step 是回溯歸屬到批次的,反正批次要完成後才會被寫出。

### 10.3 兩個檔案

| 檔案 | 位置 | 形狀 | 為什麼 |
|---|---|---|---|
| `{prefix}_Batch_*.csv` | 當日資料夾 | **寬**:一個 ratio 一欄 | 與它摘要的錄檔放在一起,day folder 複製走仍能自我解釋(`ConfigSnapshot` / `SystemLogMirror` 的同一個論點);人打開要看的就是寬的 |
| `_batches.csv` | 資料夾根 | **長**:一批一個 ratio 一列 | 看趨勢要跨幾十個批次、幾十個 day folder,而寬檔撐不住:ratio 一被新增、改名或改 class,欄位集就變了,一年的歷史會變成一堆表頭不相容的檔案 |

索引是**可從日檔重建的**,所以它是便利品而不是第二份真相。命名用前導底線,`Recording.TryParse`
會拒絕它,review 分頁會走過 —— 與 `_config_*.json`、`_log_*.csv` 同一個慣例。

某個 ratio 的 class 在該批次沒跑過:日檔留白、索引不寫那一列。拿另一段製程的數字填進去就是另一種
電漿的讀數,而這件事沒有誠實的佔位符。

### 10.4 順帶修掉的 P0 缺陷

`AdvanceStep` 原本在**第一個 gate 關閉的幀**就結束 step。光譜儀會回傳零星空白幀
(實測 13 分鐘 20 次,`SpectrumFrameDropout` 就是在數這個),所以那會在 step 中途重啟分類、
對後面碰巧出現的幀重新取一次判定,並把一支製程 step 在下游拆成好幾支。

現在容忍短於 `MaxDropoutSeconds`(1 s)的關閉 —— 與 dropout 計數器同一個門檻,更長就是電漿真的滅了。

### 10.5 引擎現在「沒有分類器時也追蹤 step」

原本 `AdvanceStep` 在沒有分類器時直接 return,`ProcessStepIndex` 恆為 0。改掉了:**plasma step
...
## 12. P3 結構(一)—— 記錄但不判定的角色(2026-09-03)

`RatioDefinition.Role`(`Alarm` / `TrendOnly` / `Guard`),預設 `Alarm`,所以既有設定不受影響。
與 `MonitorMode` 正交:`MonitorMode` 說它**量什麼**,`Role` 說它**做什麼用**。

- **`TrendOnly`** —— §4.1 的結論落地。A 與 B 的跨批次 mean/σ 是 7.3 / 6.5,低於引擎自己的接受門檻 10,
  而 §3.1 的全譜掃描確認沒有更好的含氮特徵。在那上面掛警報,是一個操作員會學會忽略的警報,
  代價比警報本身的價值高。但值仍然值得記 —— 跨批次趨勢就是從它讀出來的。
- **`Guard`** —— §4 的守衛量 `CO 607.9 / Ar 811.5`。同一套機制,相反的理由:它的工作是被**放在警報旁邊讀**。

### 12.1 新的狀態 `Observing`

電漿開著、值正在被記錄,但這一列不被判定。

刻意不用 `Normal` —— 那代表「判過了,沒問題」,而這裡沒有人判過,顯示 Normal 是一個沒有量測依據的安心。
也不用 `NoBaseline` —— 對這兩種角色來說沒有 baseline 是**預期狀態**,不是待修的故障;
所以 `ResetRuntime` 對非判定角色也讓閒置時顯示 `NoPlasma` 而不是 `NoBaseline`。

判定的停點放在**平滑之後**:值、EMA、trend 都照算,照樣進 ratio CSV 與批次記錄;門檻、確認計時器、
latch 全部不套用。這樣改角色只改變「判不判」,不改變「記什麼」。

非判定角色也被排除在 leak-rate 融合之外:`LeakRateEstimator` 把每一路當成同一個量的獨立估計,
而 trend-only 撐不起門檻、guard 量的是製程漂移。

### 12.2 守衛量進警報訊息

`LeakAlarmEventArgs.Guards` 帶上**觸發那一幀**的守衛讀數,`MainViewModel` 格式化進
`LeakMonitorState` 這筆記錄:`Leak monitor Normal → Alarm — guard: CO608/Ar811 = 0.0942 (+0.4 % of baseline)`。

**帶著走而不是事後查**,因為答案逐幀在變,而問題(「這是漏氣,還是製程氣體動了?」)問的是
警報發生的那一刻 —— 那一刻的值只有當下寫下來才留得住。偏離量必須一起寫:一週後打開 log 的人
手上不會有那份 baseline。

**報告,而不是否決。** 守衛量自己的散度是 5 %,給它否決權,遲早會壓掉一次真漏氣。
工程師同時看兩個數字三秒內能判斷;程式替他判斷,總有一次會錯而且不會說。

## 13. P3 結構(二)—— 跨批次趨勢頁(2026-09-04)

`BatchIndexReader` + `BatchTrendViewModel` + `BatchTrendPanel`,新增一個 **Batch Trend** 分頁。

這是**本計畫主要偵測機制實際被讀出來的地方**。逐幀的 Leak Monitor 頁回答「現在有沒有在漏」,
拿的是 baseline;這頁回答「這一批和前面幾十批比起來有沒有不一樣」,而在視窗會結垢的前提下,
後者是唯一問得下去的問題 —— 每一批都取自己歷程裡的同一個點(§10),所以批與批可比,
即使同一批之內的幀彼此不可比。

### 13.1 讀的是長格式索引,不是當天的寬檔

`_batches.csv`(資料夾根目錄,長格式)才是趨勢的來源。理由在 §10 已經寫過:趨勢要跨幾十個
日期資料夾,而寬檔的欄位集合在任何一個 ratio 被新增、改名或改 class 的當下就變了。長格式
schema 永遠不變,而且可以從各日檔重建 —— 它是便利品,不是第二份真相。

欄位**依標題名稱定位**(`RatioCsvReader` 對 leak-rate 欄的同一條規則),所以日後在尾端加一欄
不會讓一整年的歷史變成亂碼,只會被忽略。開檔用 `FileShare.ReadWrite`:工具在跑的時候這個檔
一直在被 append,不這樣開就讀不到 —— 這一點 `BatchCsvLoggerTests` 已經先踩過一次。

### 13.2 帶子用穩健 σ,不用標準差

`BatchSeries.Sigma = 1.4826 × MAD`。

**這是整頁唯一非做不可的設計決定。** 帶子存在的目的是讓異常看得見,而標準差會被異常本身撐開:
漏氣不會下一批就消失,連續幾批偏高的形狀正是要抓的形狀,而它們每多一批就把要抓它們的帶子拉寬一分。
`BatchIndexReaderTests` 用 12 批(9 批正常、3 批高 30 %)把兩者的差距釘住:穩健 σ 之下那 3 批
是 40 σ 外,標準差之下是 1.7 σ —— 也就是看不見。

這和 §9.3 量到的是同一個陷阱:那裡是一個 step 之內 live σ 被階躍撐大、把自己的警報門檻一起抬高;
這裡是跨批次層的同一件事,高一層而已。中位數同理:閘門會丟掉零星的空幀,但十個點取平均仍然
會被一個貼著 floor 的倖存者拉走。

### 13.3 預設歸一化,而且軸上寫清楚

三種製程的 ratio 量級差一個數量級,共用原始軸會讓小的那條完全看不見,所以每一路除以**自己的中位數**,
Y 軸就叫 `× own median`。與 Recordings 頁 `NormalizeTrend` 同樣的手法、同樣的理由,而且原始值只差一個
勾選框。理由和 ratio CSV 的第二欄要按內容命名是同一條:歸一化值和原始值不是同一個數,
圖上不說是哪一個,一年後沒有人讀得回來。

±3 σ 帶只在歸一化時畫,而且**整張圖只畫一條**(取最寬的那一路),不是每路一條 —— 三條互相重疊的
半透明帶說的比沒有還少。

左側每一路直接把「最新這批離自己的中心幾個 σ」寫成數字,因為那才是這頁存在的理由,
其餘都是它的脈絡。**判定規則(連續三批超過 3 σ)寫在頁面上**,而不是寫進程式:P3′ 之前
這頁不發警報,人看數字自己判斷。

### 13.4 沒有資料時不畫空圖

索引還不存在時(第一批完成前都是這個狀態),狀態列說明它會出現在哪個路徑,而圖**把座標軸關掉**:
OxyPlot 預設會畫出一段日期範圍和 0–100 的刻度,那些數字不來自任何一批。一張看起來像有資料的空圖,
比一片空白更糟。

### 13.5 驗證

- `BatchIndexReaderTests`(8 項):標題定位、尾端未知欄被忽略、壞列跳過而不是致命、
  分組排序、批數不足不成序列、**穩健 σ 不被異常批次撐開**、中位數 ≤ 0 時歸一化回 NaN、
  以及在 writer 仍持有檔案時讀得到。
- 全套 137 passed / 4 skipped。
...
## 15. P1 的替代:實驗機台受控漏氣測試(2026-09-04)

完整分析在 [`leak-test-20260904-analysis-zh-TW.md`](leak-test-20260904-analysis-zh-TW.md)。
這裡只記會改變計畫的三件事。

### 15.1 指標成立,而且是量出來的

`C:\DualOES\202609\04`,三組條件 × 8 個 dosing valve 位置,24 筆錄檔。
**N₂ 337.1 對空氣佔總氣體流量的比例(airFrac)呈線性,跨 2.5 個十倍程,過原點擬合殘差 4 %。**

| 特徵 | k = d(值/ArUV)/d(airFrac),E2 / E3 |
|---|---|
| **N₂ SPS 337.1** | **233 / 481** |
| N₂ SPS 357.7 | 139 / 269 —— 但生產機台上這個窗口讀到負值,搬不過去 |
| NO γ 236.6 | 96 / 73 |
| N₂ SPS 380.5 | 33 / 71 |
| OH 309 | ~0 —— 沒有反應 |

**Ar 750.4 / 811.5 在整個 20 倍的 N₂ 變化裡動不到 1 %**,O 777 被壓下 3～5 %。
把 Ar 當分母是對的,而且是量出來的,不是推的。

### 15.2 橋樑是流量比,不是體積 —— 而時間常數改變設計判斷

穩態下 N₂ 與 O₂ 的比例就是它們流量的比例,兩種氣體被同一套幫浦抽。
**15 L → 400 L 對穩態訊號完全沒有影響。** 體積只改變時間常數 τ = V·P/F:

| | τ |
|---|---|
| 實驗機台 | 10～13 s |
| 生產 400 L,1 torr,100 / 250 / 500 sccm | 316 / 126 / 63 s |

生產機台的電漿步驟長約 84 s。**若總流量在數百 sccm 以下,一個剛出現的漏氣在單一步驟之內
到不了穩態** —— 逐幀監看不是這個問題的正確工具,**跨批次比較才是**。
P2/P3 那一層不是備案,是主力;而且 τ ≫ 步驟長度意味著漏氣訊號會在批次之間平滑爬升,
正是「連續三批超過 3 σ」要抓的形狀。

### 15.3 外推:兩個未知數,不需要再上機

用**同一台偵測器上的 UV 氬多重線(415–436 nm)**當橋樑 —— 不能用 Ar 750,
因為實驗機台把 337 與 750 量在兩台未經輻射校正的光譜儀上,那條路徑會樂觀 3～7 倍。

生產機台製程 C 無漏時,同一套抽取幾何讀到 N₂ 337 = **144 counts**;
判準 3 × 3.1 % 要求多出 **13.4 counts**。代入實驗機台的 k:

**φ_min = 3.5×10⁻⁵ ～ 7.2×10⁻⁵**(空氣佔總氣體流量的比例)。

`Q_det = φ_min × 總流量`,而目標 `Q_min = rate-of-rise 規格 × 400 L`:

| 規格 | 目標 Q | Q_det @500 sccm(保守 k) | 餘裕 |
|---|---|---|---|
| 1 mTorr/min | 8.9e-3 | 6.1e-4 | **15 倍,明確 GO** |
| 0.3 mTorr/min | 2.7e-3 | 6.1e-4 | 4.4 倍,GO |
| 0.1 mTorr/min | 8.9e-4 | 6.1e-4 | 1.5 倍,勉強 |
| 0.03 mTorr/min | 2.7e-4 | 6.1e-4 | **NO-GO** |

> ### 現在缺的是兩個數字,不是一次機台時間
>
> 1. **生產機台製程 C 的總氣體流量(sccm)。**
> 2. **該機台實際的 rate-of-rise / 漏率驗收規格。**
>
> 兩張表交叉一次就得到 GO/NO-GO。

### 15.4 三個必須記住的限制

1. **實驗機台沒有 TEOS。** 連續光墊高與散度已經算進去了(外推用的是生產機台自己的
   讀值與自己的 σ);**沒算進去的是 k 本身** —— 含碳電漿裡漏進來的 N₂ 會有一部分變成
   CN/HCN,那會減少 N₂ 337。折損無法量化。
2. **CN 388 不是零背景指標。** 生產機台無漏時 388 nm 就有 1204 counts、散度 25 %。
   那個窗口在 TEOS 電漿裡本來就有東西。這是第四個「看起來很好、實際上不成立」的想法,
   和 §3 那三個並列。
3. **N₂ 357.7 搬不過去** —— 實驗機台上它是第二強,生產機台上它的連續視窗高於帶本身,讀到負值。
   只有 337.1 兩邊都成立。
