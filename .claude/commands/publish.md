...
3. When it finishes, verify `src/OES_Leak_Monitor/bin/Publish/win-x64/` holds
   all **21** expected files. Report the full output folder path and the `.exe`
   ...
   | Operator docs | `user-manual-zh-TW.html`, `daily-inspection-plan-zh-TW.html`, `secs-operation-sop-zh-TW.html`, `secs-acceptance-sheet-zh-TW.html`, `CONTEXT-zh-TW.html`, `golden-run-baseline-troubleshooting-zh-TW.html`, `baseline-builder-sop-zh-TW.html` |
...
- `publish.cmd` runs the same publish and then re-checks all 21 files itself, so
